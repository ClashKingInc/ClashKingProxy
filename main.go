...
func main() {
	...
	client := &http.Client{
		Timeout: time.Second * 20, // overall safeguard; per-request ctx timeout will be 20s
	...
	http.HandleFunc("/v1/", func(w http.ResponseWriter, r *http.Request) {
		...
		forward := baseURL + path
		// preserve query params, but for POST filter out fields param
		q := r.URL.Query()
		if r.Method == http.MethodPost {
			q.Del("fields")
		}
		if len(q) > 0 {
			forward = forward + "?" + q.Encode()
		}
